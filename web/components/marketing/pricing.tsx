...
function PlanCard({ planKey, locale }: { planKey: PlanKey; locale: string }) {
  ...
  const isPopular = planKey === "studio";    // Studio = phổ biến nhất (ladder giữa)
  ...
  return (
    <div className={`relative rounded-2xl border bg-background p-5 sm:p-6 flex flex-col ${
      isPopular ? "border-primary shadow-lg shadow-primary/10" : "border-border/60"
    }`}>
      {isPopular && (
        <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 rounded-full bg-primary text-primary-foreground text-[10px] font-bold uppercase tracking-wider whitespace-nowrap">
          {tShared("popular")}
        </div>
      )}
      ...
      <div className="mb-5 flex items-baseline gap-1">
        <span className="text-2xl font-bold sm:text-3xl">{displayPrice}</span>
        {!isFree && (
          <span className="text-xs text-muted-foreground sm:text-sm">{tShared("perMonth")}</span>
        )}
      </div>
